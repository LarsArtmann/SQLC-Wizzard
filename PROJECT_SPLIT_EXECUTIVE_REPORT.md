...
*   **Dependencies:** Minimal, likely only standard Go libraries and potentially common utility libraries for validation. It explicitly does *not* depend on `sqlc` itself, only on the schema *for* `sqlc` configurations.
